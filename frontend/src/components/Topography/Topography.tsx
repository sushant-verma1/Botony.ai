...
/** WebGL2 or nothing: a headless test environment, a blocked or exhausted
 *  context, an old machine. The backdrop is scenery, so the honest failure is
 *  to draw nothing and leave the page exactly as complete as it was. */
const createRenderer = (): InstanceType<typeof Renderer> | null => {
  try {
    return new Renderer({
      webgl: 2,
      alpha: true,
...
const Topography: FC<TopographyProps> = ({
  lowColor = '#5227FF',
  midColor = '#FF9FFC',
  highColor = '#FFFFFF',
  speed = 0.35,
  ...
  useEffect(() => {
    ...
    // One frame, at an explicit time. The control points have to be written
    // before anything is rendered: they start at zero, and a zero field is
    // flat — one band covering the screen rather than contours. Every render
    // goes through here so a still frame is a real frame.
    const draw = (t: number) => {
      time = t;
      ...
      for (let g = 0; g < 4; g++) {
        ...
        for (let j = 0; j < 4; j++) {
          const i = idx[j];
          arr[j] = ma * Math.sin(t * sp * Math.sin(i * msp) + i);
        }
      }
    ...
    // Reduced motion keeps the drawing and drops the animation: the still
    // frame setSize() already painted is the whole of the background.
    const motion = window.matchMedia('(prefers-reduced-motion: reduce)');
    let mayAnimate = !motion.matches;

    const tryStart = () => {
      if (mayAnimate && isVisible && isPageVisible && raf === 0) raf = requestAnimationFrame(loop);
    };
    const tryStop = () => {
      if (raf !== 0) {
    ...
    const onMotion = () => {
      mayAnimate = !motion.matches;
      if (mayAnimate) tryStart();
      else tryStop();
    };
    ...
    return () => {
      ...
      try {
        ...
        // Already detached; nothing to take back.
      }
